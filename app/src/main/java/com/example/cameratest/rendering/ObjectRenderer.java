...
public class ObjectRenderer {
    ...
    public void createOnGlThread(Context context, String objAssetName, String diffuseTextureAssetName)
        ...
        if(diffuseTextureAssetName.equals("models/selected.png")) {
            ...
            InputStream pngInputStream = new FileInputStream(path + "/" + diffuseTextureAssetName);
            textureBitmap = BitmapFactory.decodeStream(pngInputStream);
        }
        ...
        // Read the obj file.
        Obj obj;
        if(objAssetName.equals("models/selected.obj")) {
            InputStream objInputStream = context.getAssets().open(objAssetName);
            obj = ObjReader.read(objInputStream);
        }
        else {
            File path = new File(context.getFilesDir().getAbsolutePath());
            InputStream objInputStream = new FileInputStream(path + "/" + objAssetName);
            obj = ObjReader.read(objInputStream);
        }
    ...
    public void createOnGlThread(Context context, Obj obj, String diffuseTextureAssetName)
            throws IOException {
        // Compiles and loads the shader based on the current configuration.
        compileAndLoadShaderProgram(context);
        ...
        Bitmap textureBitmap;
        File path = new File(context.getFilesDir().getAbsolutePath());
        String[] name = diffuseTextureAssetName.split(" ");
        InputStream pngInputStream = new FileInputStream(path + "/models/" + name[1]);
        textureBitmap = BitmapFactory.decodeStream(pngInputStream);
